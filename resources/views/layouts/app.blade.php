...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased">
        ...
        <script>
            // Ensure ALL main navigation links use the content-load-link class for AJAX navigation
            document.querySelectorAll('nav a').forEach(link => {
                if (link.href.startsWith('{{ url('/admin') }}')) { // Only for admin links
                    link.classList.add('content-load-link');
                }
            });
            
            // Initial setup for the AJAX handlers
